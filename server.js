const http = require("http");
const fs = require("fs");
const path = require("path");

require("dotenv").config();

const PORT = process.env.PORT || 3000;
const OPENROUTER_API_KEY = process.env.OPENROUTER_API_KEY;

if (!OPENROUTER_API_KEY) {
  console.error("OPENROUTER_API_KEY가 없습니다.");
  process.exit(1);
}


// ======================================================
// JSON 요청 받기
// ======================================================

function readRequestBody(req) {
  return new Promise((resolve, reject) => {
    let body = "";

    req.on("data", (chunk) => {
      body += chunk;

      if (body.length > 30 * 1024 * 1024) {
        reject(new Error("업로드 용량이 너무 큽니다."));
        req.destroy();
      }
    });

    req.on("end", () => {
      try {
        resolve(JSON.parse(body));
      } catch (error) {
        reject(new Error("잘못된 JSON 데이터입니다."));
      }
    });

    req.on("error", reject);
  });
}


// ======================================================
// OpenRouter AI 요청
// ======================================================

async function callOpenRouter(messages) {

  const requestBody = {
    model: "openai/gpt-oss-20b:free",

    messages,

    temperature: 0.7,

    max_tokens: 16000,

    reasoning: {
      effort: "low",
      max_tokens: 1500
    }
  };


  console.log("");
  console.log("================================");
  console.log("OpenRouter 요청");
  console.log("모델:", requestBody.model);
  console.log("================================");


  const response = await fetch(
    "https://openrouter.ai/api/v1/chat/completions",
    {
      method: "POST",

      headers: {
        "Authorization": `Bearer ${OPENROUTER_API_KEY}`,
        "Content-Type": "application/json",

        "HTTP-Referer":
          "https://blog-writer-42ic.onrender.com",

        "X-Title":
          "Janmangchacha Blog Writer"
      },

      body: JSON.stringify(requestBody)
    }
  );


  const responseText = await response.text();


  if (!response.ok) {

    console.error("");
    console.error("================================");
    console.error("OpenRouter 오류");
    console.error("================================");
    console.error("상태 코드:", response.status);
    console.error(responseText);
    console.error("================================");

    throw new Error(
      `AI 요청 실패 (${response.status})`
    );
  }


  let result;

  try {
    result = JSON.parse(responseText);
  } catch (error) {

    console.error("JSON 파싱 실패");
    console.error(responseText);

    throw new Error(
      "AI 응답을 읽을 수 없습니다."
    );
  }


  console.log("");
  console.log("================================");
  console.log("OpenRouter 실제 응답 확인");
  console.log("================================");

  console.log(
    JSON.stringify(result, null, 2)
  );

  console.log("================================");


  const choice =
    result?.choices?.[0];

  const message =
    choice?.message;


  let text =
    message?.content || "";


  // content가 배열로 오는 경우
  if (Array.isArray(text)) {

    text = text
      .map((item) => {

        if (typeof item === "string") {
          return item;
        }

        return item?.text || "";
      })
      .join("");
  }


  if (typeof text !== "string") {
    text = String(text || "");
  }


  text = text.trim();


  // ====================================================
  // content가 비어있는 경우
  // ====================================================

  if (!text) {

    console.error("");
    console.error(
      "AI가 최종 본문을 반환하지 않았습니다."
    );

    console.error(
      "finish_reason:",
      choice?.finish_reason
    );

    console.error(
      "native_finish_reason:",
      choice?.native_finish_reason
    );

    console.error(
      "reasoning:",
      message?.reasoning
        ? "있음"
        : "없음"
    );

    throw new Error(
      "AI가 글 본문을 생성하지 못했습니다. 다시 시도해주세요."
    );
  }


  return text;
}


// ======================================================
// 본문 글자 수 계산
// ======================================================

function extractBodyOnly(blogText) {

  let body = blogText || "";


  // 해시태그 제거
  const hashtagIndex =
    body.lastIndexOf("#");

  if (
    hashtagIndex !== -1 &&
    hashtagIndex > body.length - 300
  ) {
    body =
      body.substring(
        0,
        hashtagIndex
      );
  }


  // 제목 제거
  const lines =
    body.split("\n");


  if (lines.length > 1) {

    const firstLine =
      lines[0].trim();

    if (
      firstLine &&
      !firstLine.startsWith("안녕하세요")
    ) {
      lines.shift();
    }
  }


  body =
    lines.join("\n");


  return body.trim();
}


// ======================================================
// 블로그 프롬프트
// ======================================================

function buildInitialPrompt(data) {

  const {
    storeName,
    location,
    visitDate,
    menu,
    memo,
    keywords,
    titleKeyword,
    tone,
    experience,
    provided,
    disclosure
  } = data;


  return `

너는 네이버 맛집 블로그 전문 작가
"잔망차차"야.

사용자가 직접 방문해서 작성한 것처럼
자연스럽고 친근한 맛집 후기를 작성한다.


======================================================
★ 가장 중요한 원칙
======================================================

사용자가 제공한 정보와
사진에서 명확하게 확인되는 정보만 사용한다.

없는 사실을 절대로 만들지 않는다.

없는 메뉴
없는 가격
없는 재료
없는 맛
없는 식감
없는 양
없는 주차
없는 시설
없는 웨이팅
없는 직원 이야기
없는 서비스
없는 동행인
없는 방문 목적

등을 절대로 만들지 않는다.

확실하지 않은 정보는
"정보 없음"이라고 작성한다.


======================================================
★ 글 분량
======================================================

반드시 실제 본문을 1,800자 이상 작성한다.

목표 분량은 약 2,000~2,500자다.

짧게 끝내지 않는다.

같은 문장을 반복해서 억지로 늘리지 않는다.

각 메뉴의 실제 경험과
사용자가 제공한 메모를 충분히 활용한다.


======================================================
★ 문단 완성도
======================================================

가장 중요하다.

문장을 쓰다가 중간에 끊지 않는다.

마지막 문단을 반드시 완성한다.

"~했어요."
"~좋았답니다."
"~추천드려요."

처럼 완전한 문장으로 끝낸다.

본문 마지막에는 반드시
자연스러운 마무리 문단과
잔망차차 인사를 작성한 뒤
해시태그로 끝낸다.


======================================================
★ 글 구조
======================================================

반드시 아래 순서대로 작성한다.


1. 제목

2. 안녕하세요 잔망차차에요! 😊
   자연스러운 도입부

3. 🌿 매장 분위기와 첫인상

4. 📜 메뉴 살펴보기

5. 🍝 주문한 메뉴별 상세 후기

6. 💡 방문 꿀팁

7. 💬 한줄평

8. 👍 좋았던 점

9. 📍 매장정보

10. 최종 후기

11. 잔망차차 마지막 인사

12. 해시태그


======================================================
★ 제목
======================================================

검색 키워드를 자연스럽게 포함한다.

너무 광고처럼 쓰지 않는다.

제목은 한 줄로 작성한다.


======================================================
★ 시작
======================================================

반드시

안녕하세요 잔망차차에요! 😊

느낌으로 시작한다.

사용자가 제공하지 않은
동행인이나 방문 목적을 만들지 않는다.


======================================================
★ 잔망차차 말투
======================================================

AI가 쓴 것처럼 딱딱하게 쓰지 않는다.

실제 네이버 블로그를 작성하는 사람처럼
자연스럽고 편하게 쓴다.

다음 표현을 자연스럽게 섞는다.

~더라고요
~했답니다
~좋았어요
~맛있었어요
~괜찮더라고요
~마음에 들었어요
ㅎㅎ
😊
💕
💖
ㅋㅋ

단, 같은 표현을 계속 반복하지 않는다.


======================================================
★ 메뉴 후기
======================================================

사용자가 실제로 주문했다고 입력한 메뉴만 작성한다.

메뉴마다 소제목을 작성한다.

사용자가 제공한 정보 안에서

음식 비주얼
맛
식감
소스
재료
먹었을 때 느낌
다른 메뉴와의 조합

등을 자연스럽게 설명한다.

확실하지 않은 정보는 만들지 않는다.


======================================================
★ 방문 꿀팁
======================================================

사용자가 제공한 확실한 정보만 작성한다.

정보가 부족하면

"현재 확인된 정보가 없어 별도로 적지 않을게요."

정도로 자연스럽게 처리한다.

새로운 사실을 만들지 않는다.


======================================================
★ 한줄평
======================================================

짧고 기억에 남게 작성한다.

본문에 실제로 나온 내용을 바탕으로 작성한다.


======================================================
★ 좋았던 점
======================================================

본문에 실제로 작성된 내용만
3~5개 정리한다.


======================================================
★ 매장정보
======================================================

반드시 아래 형식을 사용한다.

📍 매장정보

🏠 매장명 :
📍 위치 :
🚗 편의시설 :
🅿️ 주차정보 :

확실하지 않은 정보는
"정보 없음"이라고 작성한다.

방문일
메뉴
가격

은 매장정보에 넣지 않는다.


======================================================
★ 최종 후기
======================================================

매장정보 바로 아래에 작성한다.

최소 2~3개의 자연스러운 문단으로 작성한다.

단순히 메뉴를 다시 나열하지 않는다.

전체적인 음식 만족도
가장 기억에 남았던 부분
실제로 좋았던 점
분위기
메뉴 구성
자연스러운 추천

등을 활용한다.


======================================================
★ 마지막 인사
======================================================

최종 후기 다음에는

잔망차차 특유의 따뜻한 인사를
3~5줄 작성한다.

마지막 문장은 반드시 완성된 문장으로 끝낸다.


======================================================
★ 해시태그
======================================================

마지막 줄에 관련 해시태그
10~15개를 작성한다.


======================================================
★ 사용자 정보
======================================================

매장명:
${storeName || "정보 없음"}

위치:
${location || "정보 없음"}

방문일:
${visitDate || "정보 없음"}

메뉴:
${menu || "정보 없음"}

메모:
${memo || "정보 없음"}

키워드:
${keywords || "정보 없음"}

제목 키워드:
${titleKeyword || "정보 없음"}

말투:
${tone || "잔망차차 스타일"}

경험:
${experience || "정보 없음"}

제공 정보:
${provided || "정보 없음"}

협찬/고지:
${disclosure || "정보 없음"}


======================================================
★ 최종 출력 규칙
======================================================

설명하지 않는다.

AI가 작성했다는 말을 하지 않는다.

URL이나 출처를 작성하지 않는다.

프롬프트 설명을 하지 않는다.

완성된 블로그 글만 출력한다.

반드시 마지막까지 완성한다.

`;
}


// ======================================================
// 블로그 생성
// ======================================================

async function generateBlogPost(data) {

  const {
    images
  } = data;


  const imageContents =
    Array.isArray(images)

      ? images
          .filter(
            (image) =>
              typeof image === "string" &&
              image.trim()
          )
          .map((image) => ({
            type: "image_url",

            image_url: {
              url: image
            }
          }))

      : [];


  console.log("");
  console.log("================================");
  console.log("AI 블로그 글 생성 시작");
  console.log("================================");


  const initialPrompt =
    buildInitialPrompt(data);


  const initialMessages = [
    {
      role: "user",

      content: [
        {
          type: "text",
          text: initialPrompt
        },

        ...imageContents
      ]
    }
  ];


  let text =
    await callOpenRouter(
      initialMessages
    );


  let bodyText =
    extractBodyOnly(text);


  console.log(
    "1차 본문 글자 수:",
    bodyText.length
  );


  // ====================================================
  // 글자 수 부족하면 보완
  // ====================================================

  let attempt = 0;
  const maxAttempts = 2;


  while (
    bodyText.length < 1800 &&
    attempt < maxAttempts
  ) {

    attempt++;


    console.log("");
    console.log(
      `========== ${attempt}차 보완 시작 ==========`
    );


    const expandPrompt = `

아래 네이버 맛집 블로그 글을
잔망차차 스타일로 자연스럽게 완성해줘.

현재 본문은 ${bodyText.length}자다.

반드시 본문 1,800자 이상으로 만든다.

목표는 2,000~2,500자다.

중요:

기존 글에 없는 새로운 사실을 만들지 않는다.

없는 메뉴를 만들지 않는다.
없는 가격을 만들지 않는다.
없는 재료를 만들지 않는다.
없는 맛을 만들지 않는다.
없는 주차정보를 만들지 않는다.
없는 시설을 만들지 않는다.
없는 동행인을 만들지 않는다.
없는 서비스를 만들지 않는다.

사용자가 제공한 정보와
기존 글의 내용을 충분히 활용해서
문단을 자연스럽게 확장한다.

특히 최종 후기와 마지막 인사를
충분히 작성한다.

마지막 문단을 절대로 끊지 않는다.

반드시 완성된 문장으로 끝낸다.

기존 글의 제목부터 해시태그까지
전체 글을 다시 출력한다.

설명은 하지 않는다.

아래 글을 완성해줘.

--------------------------------

${text}

--------------------------------

`;


    const expandMessages = [
      {
        role: "user",

        content: [
          {
            type: "text",
            text: expandPrompt
          },

          ...imageContents
        ]
      }
    ];


    const expandedText =
      await callOpenRouter(
        expandMessages
      );


    if (
      expandedText &&
      expandedText.trim()
    ) {

      text =
        expandedText.trim();

      bodyText =
        extractBodyOnly(text);
    }


    console.log(
      `${attempt}차 보완 후 본문 글자 수:`,
      bodyText.length
    );
  }


  console.log("");
  console.log("================================");
  console.log("최종 글 생성 완료");
  console.log("최종 본문 글자 수:", bodyText.length);
  console.log("================================");
  console.log("");


  return text;
}


// ======================================================
// 서버
// ======================================================

const server =
  http.createServer(
    async (req, res) => {


      // ==================================================
      // 블로그 생성 API
      // ==================================================

      if (
        req.url === "/api/generate" &&
        req.method === "POST"
      ) {

        try {

          const data =
            await readRequestBody(req);


          console.log(
            "매장명:",
            data.storeName || "정보 없음"
          );


          const text =
            await generateBlogPost(data);


          res.writeHead(
            200,
            {
              "Content-Type":
                "application/json; charset=utf-8"
            }
          );


          res.end(
            JSON.stringify({
              text
            })
          );


        } catch (error) {

          console.error("");
          console.error("================================");
          console.error("글 생성 오류");
          console.error("================================");
          console.error(error);
          console.error("================================");


          res.writeHead(
            500,
            {
              "Content-Type":
                "application/json; charset=utf-8"
            }
          );


          res.end(
            JSON.stringify({
              error:
                error.message ||
                "글 생성 중 오류가 발생했습니다."
            })
          );
        }

        return;
      }


      // ==================================================
      // 메인 페이지
      // ==================================================

      if (
        req.url === "/" &&
        req.method === "GET"
      ) {

        const filePath =
          path.join(
            __dirname,
            "public",
            "index.html"
          );


        fs.readFile(
          filePath,
          "utf8",
          (err, data) => {

            if (err) {

              res.writeHead(500);

              res.end(
                "index.html을 찾을 수 없습니다."
              );

              return;
            }


            res.writeHead(
              200,
              {
                "Content-Type":
                  "text/html; charset=utf-8"
              }
            );


            res.end(data);
          }
        );

        return;
      }


      // ==================================================
      // CSS
      // ==================================================

      if (
        req.url === "/style.css" &&
        req.method === "GET"
      ) {

        const filePath =
          path.join(
            __dirname,
            "public",
            "style.css"
          );


        fs.readFile(
          filePath,
          "utf8",
          (err, data) => {

            if (err) {

              res.writeHead(404);

              res.end(
                "CSS 파일을 찾을 수 없습니다."
              );

              return;
            }


            res.writeHead(
              200,
              {
                "Content-Type":
                  "text/css; charset=utf-8"
              }
            );


            res.end(data);
          }
        );

        return;
      }


      // ==================================================
      // JavaScript
      // ==================================================

      if (
        req.url === "/script.js" &&
        req.method === "GET"
      ) {

        const filePath =
          path.join(
            __dirname,
            "public",
            "script.js"
          );


        fs.readFile(
          filePath,
          "utf8",
          (err, data) => {

            if (err) {

              res.writeHead(404);

              res.end(
                "JavaScript 파일을 찾을 수 없습니다."
              );

              return;
            }


            res.writeHead(
              200,
              {
                "Content-Type":
                  "application/javascript; charset=utf-8"
              }
            );


            res.end(data);
          }
        );

        return;
      }


      // ==================================================
      // 404
      // ==================================================

      res.writeHead(
        404,
        {
          "Content-Type":
            "text/plain; charset=utf-8"
        }
      );


      res.end(
        "페이지를 찾을 수 없습니다."
      );
    }
  );


// ======================================================
// 서버 실행
// ======================================================

server.listen(
  PORT,
  () => {

    console.log("");
    console.log("================================");
    console.log(
      "블로그 초안 생성기가 실행되었습니다."
    );
    console.log(
      `포트: ${PORT}`
    );
    console.log("================================");
    console.log("");
  }
);
